...
function mousedown(event) {
    ...
    if (event.target.classList.contains('move-btn')) {
        ...
        duplicateOfSelectedToDoItem.setAttribute('style', 'position:fixed;top:' + todoListItemCoordinates.mouseY + 'px;left:' + todoListItemCoordinates.mouseX + 'px;max-width:370px;background-color:lightgrey;opacity:0.6;display:none');
        body.appendChild(duplicateOfSelectedToDoItem);
        duplicateOfSelectedToDoItem.style.display = 'block';
        duplicateOfSelectedToDoItem.style.left = elementClientRect.left + 'px';
        duplicateOfSelectedToDoItem.style.top = elementClientRect.top + 'px';
